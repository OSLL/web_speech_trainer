...
from app.pdf_parser.counters.bi_gram import BiGramCounter


class TextComporatorOnBiGramCounter(BaseTextComporator):
    def __init__(self):
        super(TextComporatorOnBiGramCounter, self).__init__()
        self.__meta = BaseMeta()

    def cosine_compare(self, text1, text2, get_text1_as_sample=True):
        self._counter1 = BiGramCounter(text=text1).get_counter()
        self._counter2 = BiGramCounter(text=text2).get_counter()
        ...
        print('On BiGram Compare : Meta : text 1 : ', self.__meta.text1_token_weights)
        print('On BiGram Compare : Meta : text 2 : ', self.__meta.text2_token_weights)
