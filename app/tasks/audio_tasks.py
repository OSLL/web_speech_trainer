...
from app.mongo_odm import DBManager, TrainingsDBManager, RecognizedAudioToProcessDBManager
from app.status import AudioStatus
from app.root_logger import get_root_logger

logger = get_root_logger('celery_audio_task')
...
def recognize_audio_task(self, training_id, presentation_record_file_id):
    ...
    """
    logger.info(f'Starting recognize_audio_task for training_id={training_id}, file_id={presentation_record_file_id}')
    try:
        # Обновляем статус
        ...
        if presentation_record_file is None:
            raise Exception(f'Presentation record file {presentation_record_file_id} not found')
