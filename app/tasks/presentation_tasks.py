...
from app.mongo_odm import DBManager, TrainingsDBManager, RecognizedPresentationsToProcessDBManager, PresentationFilesDBManager
from app.presentation_recognizer import PRESENTATION_RECOGNIZERS
from app.status import PresentationStatus
from app.root_logger import get_root_logger

logger = get_root_logger('celery_presentation_task')
...
def recognize_presentation_task(self, training_id, presentation_file_id):
    ...
    """
    logger.info(f'Starting recognize_presentation_task for training_id={training_id}, file_id={presentation_file_id}')
    try:
        # Обновляем статус
        ...
        if presentation_file_info is None:
            raise Exception(f'Presentation file info for {presentation_file_id} not found')
        ...
        # Получаем файл
        file_id_to_fetch = presentation_file_id if not nonconverted_file_id else nonconverted_file_id
        ...
        RecognizedPresentationsToProcessDBManager().add_recognized_presentation_to_process(recognized_presentation_id, training_id)
        TrainingsDBManager().change_presentation_status(training_id, PresentationStatus.SENT_FOR_PROCESSING)
        
        logger.info(f'Finished recognize_presentation_task for training_id={training_id}')
        ...
        TrainingsDBManager().append_verdict(training_id, f'Presentation recognition failed after retries: {e}')
