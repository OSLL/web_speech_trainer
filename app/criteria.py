...
# TODO:
# 1. Refactor class
# 2. Add Parselmouth package to requirements.txt
class IntonationStabilityCriterion(Criterion):
    CLASS_NAME = 'IntonationStabilityCriterion'
    ...
    def __init__(self, parameters, dependent_criteria):
        super().__init__(
            name=IntonationStabilityCriterion.CLASS_NAME,
            parameters=parameters,
            dependent_criteria=dependent_criteria,
        )
    ...
    def apply(self, audio: Audio, presentation: Presentation, training_id: ObjectId, criteria_results: dict) \
        ...
        unstables_num = (pitch_critical_maximas.size + pitch_critical_minimas.size) / 2
        if (unstables_num >= 10):
            return CriterionResult(result=0)
        return CriterionResult(result=1)